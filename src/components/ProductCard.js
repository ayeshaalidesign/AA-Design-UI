        ...
        {products.map((product, index) => (
          <div key={index} className="flex flex-col">
           ...
           <img src={product.imageUrl} alt={product.productName} onClick={() => handleRedirection(product.productId)} className="w-full h-full mt-6 mb-6 object-cover rounded-md" />
            ...
            <button onClick={() => handleRedirection(product.productId)} className='mt-6 text-md font-semibold text-white bg-black px-2 py-2 w-full mb-6 font-verdana'>Shop</button>
           </div>
           <div>
           </div>
          </div>
